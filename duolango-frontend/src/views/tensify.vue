<template>
  <div>
    ...
    <div v-else>
      <h1>{{ sentence }} ({{ verb }}).</h1>
      <button v-for="(option, index) in verbOptions" :key="index" @click="checkVerb(option)"
  ...
  setup() {
    ...
    const makeApiRequest = () => {
      ...
      if (proxy.global.lang == "Spanish") {
        apiLink = "https://phrase-generation.vercel.app/en_es_verbs/"
      }
      else {
        apiLink = "https://phrase-generation.vercel.app/en_swa_verbs/"
      }
      var config = {
        method: 'get',
        url: apiLink,
